function getRandomIdFromArray(a){return Math.floor(Math.random()*a.length)}function getRandomValueFromArray(a){return a[getRandomIdFromArray(a)]}function printAsciiById(a){const r=asciis[a];if(!r)return console.warn(`ASCII not found for id: ${a}`),null;console.log(`%c${r.art}`,`color: ${r.color}; font-family: monospace;`)}function printAsciiByName(a){const r=asciis.findIndex(r=>r.name===a);if(-1===r)return console.warn(`ASCII not found for name: ${a}`),null;printAsciiById(r)}function printRandomAscii(a="all"){var r=asciis.filter(r=>r.type===a);r.length<1&&(r=Object.values(asciis));const n=getRandomValueFromArray(r);printAsciiByName(n.name)}var asciis=[{type:"animal",name:"anteater",art:String.raw` 
       _.---._    /\\
    ./'       "--'\//
  ./              o \
 /./\  )______   \__ \
